...
def test_trainer_can_view_class_members(client, trainer_headers, seeded_trainer, seeded_class):
    response = client.get(f"/api/classes/{seeded_class['_id']}/members", headers = trainer_headers)
...
def test_member_can_view_their_class_members(client, app, seeded_class, seeded_member, member_headers):
    ...
    with app.app_context():
        ...
        resource.book_class(seeded_class["_id"], seeded_member["_id"])

    response = client.get(f"/api/classes/{seeded_class["_id"]}/members", headers=member_headers)
    assert response.status_code == 200
    data = response.get_json()
    assert len(data["members"]) == 1 # the class list should contain only themselves, which is the entire list
    
    
def test_unauthorized_user_cannot_view_members(client, seeded_class):
    response = client.get(f"/api/classes/{seeded_class['_id']}/members")
...
def test_view_nonexistent_class_members(client, trainer_headers):
    response = client.get("/api/classes/invalid_id/members", headers = trainer_headers)
